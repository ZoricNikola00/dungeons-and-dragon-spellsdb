  ...
  if (favorite?.length === 0) {
    return (
      <div className='text-center text-white'>
        ...
        <button
          className='text-2xl border border-[#666633] transition duration-500 hover:bg-white hover:text-[#666633] bg-[#666633] p-4 tracking-wider rounded-lg'
          onClick={() => nav('/spells')}
        >
          Check the spells
        </button>
  ...
  return (
    <div className='w-[90%] md:w-[80%] mx-auto text-white'>
      <div className='flex items-center justify-between'>
        ...
        <button
          className='text-lg px-2 border border-[#666633] transition duration-500 hover:bg-white hover:text-[#666633] bg-[#666633] py-1 tracking-wider rounded-lg'
          onClick={() => nav('/spells')}
        >
          Check other spells
        </button>
      ...
      {favorite?.map((spell) => (
        <div
          key={spell.index}
          className='relative items-center w-full mb-3 text-lg font-bold bg-green-300/30 rounded p-4 grid grid-cols-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-7 xl:grid-cols-8'
        >
          ...
          <button
            data-testid='remove'
            onClick={() => dispatch(addFavorite(favorite.find((fav) => fav.index === spell.index)))}
            className='absolute text-3xl right-[20px] z-30 hover:text-red-500 duration-500 transition'
          >
            <TiTimes />
          </button>
          <button
            onClick={() => nav(`/spells/${spell.index}`)}
            className='absolute text-3xl right-[60px] z-30 hover:text-[#666633] transition duration-500'
          >
            <GiScrollUnfurled />
          </button>
        </div>
