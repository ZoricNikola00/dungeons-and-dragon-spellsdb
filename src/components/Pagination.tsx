  ...
  return (
        <ul className='text-white flex items-center justify-center text-lg md:text-3xl mb-4 md:mb-0'>
            {pageNumbers.map(number=>(
                <li key={number} onClick={()=>setCurrentPage(number)} className={`${currentPage===number?'bg-[#666633]':'bg-transparent'} rounded p-3 md:p-4 hover:bg-[#666633] cursor-pointer m-1 md:m-2`}>
                    {number}
                </li>
            ))}
        </ul>
